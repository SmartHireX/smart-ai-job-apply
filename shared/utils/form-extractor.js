...
class FormExtractor {
    ...
    extract(root) {
        ...
        allFields.forEach(el => {
            ...
            // We only want ONE field object per group (the first one encountered)
            if (el.type === 'radio' || el.type === 'checkbox') {
                const groupKey = this.getGroupKey(el);
                if (processedGroups.has(groupKey)) return;
                processedGroups.add(groupKey);
            }
    ...
    /**
     * Generate unique key for a radio/checkbox group
     */
    getGroupKey(input) {
        if (input.name) return `name:${input.name}`;
        // Fallback for anonymous groups
        const wrapper = input.closest('.form-group, fieldset, tr, .radio-group, .checkbox-group, div[role="group"]');
        if (wrapper && wrapper.id) return `wrapper:${wrapper.id}`;
        return `element:${input.id || Math.random()}`; // Weak fallback
    }
}
