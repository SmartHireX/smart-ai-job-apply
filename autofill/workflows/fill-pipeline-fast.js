        ...
        for (const field of unmapped) {
            ...
            if (fieldLabel.length > 2) {
                if (window.SelectionCache && window.SelectionCache.getCachedValue) {
                    ...
                    // Use centralized FuzzyMatcher for intelligent cache lookup
                    const matchResult = window.FuzzyMatcher?.match(fieldLabel, smartMemory, { threshold: 0.5 });

                    if (matchResult?.success) {
                        foundAnswer = matchResult.value;
                    }
                }
            }
